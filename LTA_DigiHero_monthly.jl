...
n_states = 3
sim_no = string("2026_07_20/", n_states, "states/age_sex_comorbidity_covs_datafusion_monthly")
combined_all = CSV.read(sim_dir * "2026-07-20_data_symptoms_DigiHero_Bonn_preprocessed.csv", DataFrame, missingstring=["", "NA"]);
...
function add_dummy_symptom_cols(combined_all)
    for timepoint in 2:30
        if timepoint % 3 == 0
            continue
        else 
            # Add missing timepoint symptom columns
            for symptom in symptoms
                combined_all[!, "pcc_"*string(timepoint)*"months_sym_"*symptom] .= -1
            end
        end
    end
...
LTA.savefig_auto(fig_em, "outputs/" * sim_no * "/results/plots/emission_matrix.png")
...
fig1 = LTA.compare_estimation_2_data_fup(estimation_output; T=31, symptom_names=symptoms, type="best_fitted", observation_type="bernoulli")
fig1_with_CI = LTA.compare_estimation_2_data_with_CIs_monthly(estimation_output; T=31, symptom_names=symptoms, type="best_fitted", observation_type="bernoulli")
...
fig = LTA.ensemble_uq_monthly(estimation_output; T=31, symptom_names=symptoms, observation_type="bernoulli", num_multistarts=2)
LTA.savefig_auto(fig, "outputs/" * sim_no * "/results/plots/ensemble_uq.png")

fig = LTA.estimated_states_over_time(estimation_output; T=31, type="best_fitted", n_states=n_states)
